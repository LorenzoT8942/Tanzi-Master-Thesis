...
class Simulation3D():
    ...
    def __init__( self ):
        ...
        self.rugby_elongation = 1.5 # rapporto tra asse maggiore e minore della sfera, per trasformarla in una ellissoide allungata (rugby ball)
        ...
        # MESH
        self.circle_seed_size          = 0.5 # dimensione degli elementi della mesh sulla palla, in mm.
    ...
    def _saveInputDataToFile(self):
        ...
        inputData = { "index"                 : self.index,
                      ...
                      "circle_origin_x"       : self.circle_origin_x,
                      "circle_origin_y"       : self.circle_origin_y,
                      "circle_origin_z"       : self.circle_origin_z,
                      "ball_long_axis"        : self.ball_long_axis }
    ...
    def runSimulation( self,
        ...
        self.spin_X = SPIN_X * 2 * math.pi # convert rotations per second to radians per second
        self.spin_Y = SPIN_Y * 2 * math.pi
        self.spin_Z = SPIN_Z * 2 * math.pi
        ...
        #computes the length of the long axis of the rugby ball based on the radius and the elongation factor
        self.ball_long_axis = self.circle_radius * self.rugby_elongation
        ...
        container_dir = os.path.join(parent_dir, 'NewSimulations_RUGBY')
        ...
        sketch_circle = model.ConstrainedSketch( name      = 'sketch-circle', 
                                                 sheetSize = self.CIRCLE_BASE_RADIUS )
        ...
        omega_magnitude = math.sqrt(self.spin_X**2 + self.spin_Y**2 + self.spin_Z**2)

        center_point = (self.circle_origin_x, self.circle_origin_y, self.circle_origin_z)

        if omega_magnitude == 0:
            axis_direction = (0, 1, 0) # asse Y di default
        else:
            # Normalizziamo il vettore direzione
            axis_direction = (self.spin_X / omega_magnitude,    
                              self.spin_Y / omega_magnitude, 
                              self.spin_Z / omega_magnitude)    
            
        end_point = (center_point[0] + axis_direction[0], 
                     center_point[1] + axis_direction[1], 
                     center_point[2] + axis_direction[2])
        ...
        velocity = model.Velocity( name      = "Velocity", 
                                   ...
                                   omega = omega_magnitude,
                                   axisBegin=center_point,
                                   axisEnd=end_point
                                   )
        ...
        for n in range(0,len(part_circle.nodes)):
            ...
            # Applichiamo lo scaling base a tutti gli assi per avere la dimensione corretta.
            # IN PIU', moltiplichiamo l'asse Y per il fattore rugby per allungarla.
            # (L'asse Y è l'asse di rivoluzione nel tuo sketch, quindi è quello "lungo")
            
            new_x = coords[0] * scale
            new_y = coords[1] * scale * self.rugby_elongation
            new_z = coords[2] * scale

            # Modifica il nodo
            part_circle.editNode(
                coordinate1=new_x, 
                coordinate2=new_y,
                coordinate3=new_z,
        ...
        # ------------ SAVING BALL COORDINATES OVER TIME ------------ 
        if SAVEBALLCOORDINATES:
            
            log("Extracting Reference Point kinematics (Fastest & Lightest)...")
            rp_output_filename = os.path.join(self.new_path, str(self.index) + '_output_ball_kinematics.csv')
            
            try:
                # Recupera il Node Set del Reference Point a livello di Assembly
                rp_node_set = odb.rootAssembly.nodeSets['CIRCLE-RP']
                
                with open(rp_output_filename, 'w') as f:
                    # Intestazione aggiornata: X, Y, Z assolute e Rotazioni
                    f.write("Time,X_Abs,Y_Abs,Z_Abs,UR1,UR2,UR3\n")
                
                for frame in odb.steps['Step-1'].frames:
                    frame_time = frame.frameValue
                    
                    # Estrai i campi U e UR solo per l'RP
                    u_field = frame.fieldOutputs['U'].getSubset(region=rp_node_set)
                    ur_field = frame.fieldOutputs['UR'].getSubset(region=rp_node_set)
                    
                    # Il Reference Point è un singolo nodo, prendiamo i valori (se esistono)
                    u_vals = u_field.values[0].data if len(u_field.values) > 0 else (0.0, 0.0, 0.0)
                    ur_vals = ur_field.values[0].data if len(ur_field.values) > 0 else (0.0, 0.0, 0.0)
                    
                    # Calcola le coordinate assolute sommando lo spostamento all'origine nota
                    abs_x = self.circle_origin_x + u_vals[0]
                    abs_y = self.circle_origin_y + u_vals[1]
                    abs_z = self.circle_origin_z + u_vals[2]
                    
                    with open(rp_output_filename, 'a') as f:
                        f.write(f"{frame_time},{abs_x},{abs_y},{abs_z},{ur_vals[0]},{ur_vals[1]},{ur_vals[2]}\n")

                log(f"Saved absolute ball kinematics to {rp_output_filename}")
                
            except KeyError:
                log("ERROR: Could not find 'CIRCLE-RP' node set in the ODB. Make sure the set is correctly generated.")
